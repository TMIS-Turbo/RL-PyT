...
class Agent():
    ...
    def train_model(self):
        ...
        if 1:  # Check shape of experiences
    ...
    def select_action(self, state):
        ...
        log_pi = dist.log_prob(z).sum(dim=-1)
        log_pi -= torch.sum(torch.log(1 - pi_tensor.pow(2) + 1e-6), dim=-1)
    ...
    def run(self, max_step):
        ...
        while not (done or step_number == max_step):
            ...
            if self.eval_mode:
                action, _, _ = self.select_action(torch.Tensor(obs).to(device))
