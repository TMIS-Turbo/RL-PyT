...
class ActorNet(nn.Module):
    ...
    def forward(self, x):
        ...
        log_sigma = torch.clamp(self.sigma_head(x), -10, 10)
...
class agent(nn.Module):
    ...
    def select_action(self, s):
        ...
        prob = prob.item()
        a = Range*action
        # print("---Range*action---", a)
        return a, prob

    def load_model(self):
        anet = torch.load('/home/ppo_anet.pkl')
        cnet = torch.load('./model/ppo_cnet1.pkl')

    def save_model(self):
        torch.save(self.anet, './model/ppo_anet1.pkl')
        torch.save(self.cnet, './model/ppo_cnet1.pkl')
    ...
    for n_epi in range(traning_step):
        ...
        while not done:
            for t in range(T_horizon):
                ...
                model.put_data((s, a/Range, (r+8)/8, s_prime, prob, done))
                s = s_prime
