...
learning_rate_alpha = 0.005   # 0.001
gamma         = 0.95   # 0.95
lmbda         = 0.95
eps_clip      = 0.1
Range         = 2
...
class agent(nn.Module):
    ...
    def train_net(self):
        ...
        for i in range(K_epoch):
            # _, _, next_log_pi = self.select_action(np.array(s_prime, dtype=float), mode='train')
            mu, sigma = self.anet(s_prime)
            dist = Normal(mu, sigma)
            a_next = dist.rsample()   # reparameterization trick (mean + std * N(0,1))
            a_next = a_next.clamp(-Range, Range)
            prob = dist.log_prob(a_next).exp()
            next_log_pi = self.tsallis_entropy_log_q(prob, self.q)
            next_log_pi = next_log_pi.unsqueeze(0).t()

            td_target = r + gamma * (self.cnet(s_prime) - self.alpha * next_log_pi) * done_mask
            ...
            # _, _, log_pi = self.select_action(s, mode='train')
            a_ = dist.rsample()
            a_ = a_.clamp(-Range, Range)
            prob = dist.log_prob(a_).exp()
            log_pi = self.tsallis_entropy_log_q(prob, self.q)
            log_pi = log_pi.unsqueeze(0).t()
            # print("log_pi", log_pi, torch.min(surr1, surr2))

            loss_a = self.alpha * log_pi - torch.min(surr1, surr2)
    ...
    def select_action(self, s, mode):
        ...
        if mode == "test":
            ...
            prob = dist.log_prob(action).exp()
            action = action.clamp(-Range, Range)
            # log_prob = self.tsallis_entropy_log_q(prob, self.q)
            # print("#action & prob & log_prob#", action.shape, prob.shape, log_prob.unsqueeze(0).t().shape)
    ...
    def load_model(self):
        self.anet = torch.load('./model_v/1205/v_anet0.pkl')
        # self.cnet = torch.load('./model_v_new/v_cnet0.pkl')

    def save_model(self):
        torch.save(self.anet, './model_v/1205/v_anet0.pkl')
        # torch.save(self.cnet, './model_v_new/v_cnet0.pkl')
    ...
    for n_epi in range(traning_step):
        ...
        while not done:
            for t in range(T_horizon):
                a, prob = model.select_action(s, mode='train')
