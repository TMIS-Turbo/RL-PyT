...
parser.add_argument('--algo', type=str, default='asac', help='select an algorithm among sac, asac')
...
class Agent():
    ...
    def __init__(self,
        ...
        self.qf1 = CriticNet(state_dim).to(device)
        self.qf2 = CriticNet(state_dim).to(device)
        # Target network
        self.qf1_target = CriticNet(state_dim).to(device)
        self.qf2_target = CriticNet(state_dim).to(device)
    ...
    def train_model(self):
        ...
        # print("loss", actor_loss, qf1_loss, qf2_loss, alpha_loss)
    ...
    def tsallis_entropy_log_q(self, x, q):
        ...
        return log_q_x.sum(dim=-1).unsqueeze(1)
    ...
    def run(self, max_step):
        ...
        while not (done or step_number == max_step):
            ...
            if self.eval_mode:
                action = self.select_action_single(torch.Tensor(obs).to(device), mode='test')
